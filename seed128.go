...
// Common functions
func getB0(a int) int { return 0x000000ff & (a) }
func getB1(a int) int { return 0x000000ff & int(uint(a)>>8) }
func getB2(a int) int { return 0x000000ff & int(uint(a)>>16) }
func getB3(a int) int { return 0x000000ff & int(uint(a)>>24) }
func endiansChange(dws *[]int) {
	(*dws)[0] = int(uint((*dws)[0])>>24) | ((*dws)[0] << 24) | (((*dws)[0] << 8) & 0x00ff0000) | int((uint((*dws)[0])>>8)&0x0000ff00)
}
func endianChange(dws int) int {
	return int(uint(dws)>>24) | (dws << 24) | ((dws << 8) & 0x00ff0000) | (int(uint(dws)>>8) & 0x0000ff00)
}

// SEED round function
func seedRound(l0, l1, r0, r1, k *[]int) {
	var t0, t1 int
	var t00, t11 int64
	...
	t1 ^= t0
	if t0 < 0 {
		t00 = int64(t0&0x7fffffff) | int64(0x80000000)
	} else {
		t00 = int64(t0)
	}
	t1 = ss0[getB0(t1)] ^ ss1[getB1(t1)] ^ ss2[getB2(t1)] ^ ss3[getB3(t1)]
	if t1 < 0 {
		t11 = int64(t1&0x7fffffff) | int64(0x80000000)
	} else {
		t11 = int64(t1)
	}
	t00 += t11
...
func SeedEncrypt(pbData []byte, pdwRoundKey []int) (outData []byte) {
	...
	for i := 0; i < 4; i++ {
		outData[i] = byte((int(uint(r0[0]) >> uint(8*(3-i)))) & 0xff)
		outData[4+i] = byte((int(uint(r1[0]) >> uint(8*(3-i)))) & 0xff)
		outData[8+i] = byte((int(uint(l0[0]) >> uint(8*(3-i)))) & 0xff)
		outData[12+i] = byte((int(uint(l1[0]) >> uint(8*(3-i)))) & 0xff)
	}
	return
}
...
func encRoundKeyUpdate0(k, a, b, c, d *[]int, z int) {
	var t0, _, t00, t11 int
	t0 = (*a)[0]
	(*a)[0] = int(uint((*a)[0])>>8) ^ ((*b)[0] << 24)
	(*b)[0] = int(uint((*b)[0])>>8) ^ (t0 << 24)
...
func encRoundKeyUpdate1(k, a, b, c, d *[]int, z int) {
	var t0, _, t00, t11 int
	t0 = (*c)[0]
	(*c)[0] = int(uint((*c)[0])>>8) ^ ((*d)[0] << 24)
	(*d)[0] = int(uint((*d)[0])>>8) ^ (t0 << 24)
	t00 = int((*a)[0]) + int((*c)[0]) - int(kc[z])
	t11 = int((*b)[0]) + int(kc[z]) - int((*d)[0])
	(*k)[0] = ss0[getB0(int(t00))] ^ ss1[getB1(int(t00))] ^ ss2[getB2(int(t00))] ^ ss3[getB3(int(t00))]
	(*k)[1] = ss0[getB0(int(t11))] ^ ss1[getB1(int(t11))] ^ ss2[getB2(int(t11))] ^ ss3[getB3(int(t11))]
